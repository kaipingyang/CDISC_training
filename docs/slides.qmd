...
`pharmaverseraw::ae_raw` —— 字段名和取值都是"人话"（EDC 直接导出的样子）：

```{r}
#| eval: TRUE
#| echo: FALSE
#| message: FALSE
cat("DEBUG_LIBS:", paste(.libPaths(), collapse = " || "), "\n")
cat("DEBUG_R_LIBS:", Sys.getenv("R_LIBS"), "\n")
...
reactable::reactable(
  ...
  columns = list(
    STUDY      = reactable::colDef(header = "研究"),
    PATNUM     = reactable::colDef(header = "受试者"),
    IT.AETERM  = reactable::colDef(header = "不良事件（人话）"),
    IT.AESEV   = reactable::colDef(header = "严重程度（人话）"),
    AEOUTCOME  = reactable::colDef(header = "结局（人话）")
  ),
  height = 340, highlight = TRUE
)
```
...
**产物**：符合 SDTM 规范的 DM 数据集（STUDYID / USUBJID / AGE / SEX / RACE…），
导出为 `dm.xpt`（SAS 传输文件，写入 `sdtm/output/`）。
...
rtables::as_html(head(tbl_ae, 12))
```
