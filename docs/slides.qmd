...
**skill = 把领域经验打包成"说明书"**：Claude 读到说明书，就按里面的套路干活。

**一个 skill 的结构**（以 `sdtm-map` 为例）：

```text
sdtm-map/
├── SKILL.md        ← 说明书：什么时候触发 + 怎么用 + 代码模板
└── references/     ← 资料：函数速查、常见坑记录
```

**它怎么一步步设计出来的**：

1. **收集** —— 学员常问的问题（"怎么把 `IT.SEX` 变成 `SEX`？"）
2. **提炼** —— 归纳成"域映射套路"：选函数（`assign_no_ct` / `assign_ct` / `hardcode_ct`）→ 填参数
3. **成文** —— 写进 SKILL.md：触发词（"生成 DM 域"）+ 可运行模板
4. **实测** —— 跑通模板，确认产出合规
5. **回填** —— 实战踩的坑记进 references，下次自动避开

::: {.callout-note}
你不需要会写 skill —— 但看懂结构后，就明白为什么说"生成 DM 域"会自动加载 `sdtm-map`。
:::
...
![](images/sdtm_flow.svg){width="100%" fig-align="center" fig-alt="原始采集数据经三种映射动作变成 SDTM 域"}
...
![](images/adam_hub.svg){width="72%" fig-align="center" fig-alt="SDTM 域合并出 ADSL，ADSL 派生各 ADaM 并作为 TFL 百分比分母"}
...
`tern::g_km()` 返回 **ggplot 对象**，幻灯片里直接渲染 —— 与 `tfl/03_g_km.R` 完全同源。
...
::: {.column width="55%"}
![](images/app.png){width="100%" fig-align="center" fig-alt="teal 交互 app 的人口学特征表模块：左侧参数面板、右侧统计表"}
:::
