...
| **③ 文件引用** | "参考我写好的 `users/我的名字/sdtm/dm.R` 帮我写 VS 域" | 让 AI 照已有脚本仿写 |
...
"帮我补全这两个 TODO，先提示思路，别直接给完整代码，也别读 `sdtm/vs.R`"。练完再对照 `sdtm/vs.R` 自查。
:::
...
::: {.callout-note}
完整实现见 `sdtm/ae.R`（含 SUPPAE），练完再对照。先练熟 DM 和 VS，再挑战这个域。
:::
