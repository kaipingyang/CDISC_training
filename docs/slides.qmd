...
2. **skill 触发** —— 说到领域关键词（如"生成 AE 域"），对应 skill 自动加载，按 CDISC 规范写代码
...
| 动作 | 你怎么说 | 场景 |
|------|---------|------|
| **① 对话生成** | "帮我生成 AE 域的 SDTM 映射代码" | 从零起一个脚本 |
| **② 显式喊 skill** | "用 sdtm-map skill 做 VS 域" | 关键词没自动触发时 |
| **③ 文件引用** | "参考 `sdtm/ae.R` 帮我写 `vs.R`" | 让 AI 照已有脚本仿写 |
| **④ 迭代修改** | "把导出路径改成 tempdir()" | 生成后继续调 |
