...
> ⚠️ **模板定位说明**：以下模板为**简化示意**（通用 pharmaverse 数据、变量名直拷），
> 目的是展示 sdtm.oak 的映射套路。本项目实际生成时，**必须以项目数据风格为准**：
> 原始数据用 `pharmaverseraw` 包（变量带 `IT.` 前缀）、管道起点先建 oak 骨架、
> 受控术语查 `metadata/sdtm_ct.csv`——最终代码应能直接对应 `users/_template/sdtm/`
> 的 TODO 填空（参考上方"项目本地参考脚本"段落）。
...
生成代码时**以本 skill 模板 + `metadata/` 规格书（onco_spec.xlsx、sdtm_ct.csv）为准**；
项目根 `sdtm/` `adam/` `tfl/` 是**完整答案脚本**，遵守 CLAUDE.md 答案访问规则：

- **练习模式**（学员请求生成代码、补全 TODO、评估正确性）：答案脚本**严禁读取**，
  只依据 metadata 规格书与 CDISC 标准生成；代码风格对齐
  `users/_template/sdtm/`（pharmaverseraw 数据 + `IT.` 前缀原始变量 + oak 骨架）。
- **检查模式**（学员明确说"检查/对照/对比答案"）：才可读取答案脚本，逐条比对说明差异。
