...
vs_sysbp <-
  # TODO 1: 用 hardcode_ct 给主题变量 VSTESTCD 赋固定值 "SYSBP"
  #   raw_var = "SYS_BP"，tgt_var = "VSTESTCD"，tgt_val = "SYSBP"，
  #   ct_spec = study_ct，ct_clst = "C66741"（这里是管道开头，不写 id_vars）
  # 👉 在这里补一段 hardcode_ct(...)，参考下面舒张压 vs_diabp 的第一段 hardcode_ct 写法
...
vs_sysbp <- vs_sysbp %>%
  ...
  # TODO 2: 映射 VSORRES（原始结果值 / 收缩压读数），原始变量 SYS_BP → 目标 VSORRES
