...
# 参考答案：sdtm/dm.R（完整版，别改它）—— 先自己填，卡住再看。
# =============================================================================
...
dm <- dm %>%
  # TODO 1: 映射 AGE（年龄），原始变量 IT.AGE → 目标 AGE
  #   用 assign_no_ct（年龄是原始数值，无需受控术语转换，直接原样复制），id_vars = oak_id_vars()
  # 👉 在这里补一段 assign_no_ct(...)，参考下面 ARMCD 的 assign_no_ct 写法
  ...
  # TODO 2: 映射 SEX（性别），原始变量 IT.SEX → 目标 SEX
  #   用 assign_ct（性别需按受控术语转换，如"男"→"M"），受控术语 codelist "C66731"，id_vars = oak_id_vars()
  # 👉 在这里补一段 assign_ct(...)，参考下面 ETHNIC / RACE 的 assign_ct 写法
  identity() %>%   # 占位：填好上面 TODO 后删掉这行 identity()
...
dm <- dm %>%
  # TODO 3: 派生 RFSTDTC（研究参考起始日期），从配置表 ref_date_conf_df 计算
  #   用 oak_cal_ref_dates，der_var = "RFSTDTC"，min_max = "min"（取最早日期），
  #   ref_date_config_df = ref_date_conf_df，raw_source = list(ec_raw=ec_raw, ds_raw=ds_raw, dm_raw=dm_raw)
  # 👉 在这里补一段 oak_cal_ref_dates(...)，参考下面 RFENDTC 的写法（只是 min_max 换成 "max"）
  identity()   # 占位：填好上面 TODO 后删掉这行 identity()
