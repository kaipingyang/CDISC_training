...
advs <- advs %>%
  ...
  # TODO 1: 给每条记录赋予分析参数代码 PARAMCD
  #   用 derive_vars_merged_lookup，按 VSTESTCD 从 param_lookup 查出对应 PARAMCD。
  #   要点：
  #     - dataset_add = param_lookup
  #     - new_vars    = exprs(PARAMCD)
  #     - by_vars     = exprs(VSTESTCD)
  #     - print_not_mapped = TRUE（打印没匹配上的 VSTESTCD，便于核查）
  # 👉 把下面的 identity() 换成 derive_vars_merged_lookup(...)
  identity()   # 占位：填好上面 TODO 后改成 derive_vars_merged_lookup(...)
...
# TODO 4（理解题，不用改代码）：LOV 的 EOT 窗口为什么限定在 Week 6-12？
#   按方案 EOT 访视为 Week 12；提前结束的受试者取窗口内最后一次已参加访视
#   （Week 6/8）；未达到 Week 6 的受试者不做 LOV。想一想为什么这样设计——
#   对照检查时可以让 Claude 给你讲解，或查 CDISC ADaM 关于分析访视的说明。
...
# TODO 2: 派生基线值 BASE
#   用 derive_var_base，把每个 BASETYPE 内 ABLFL=="Y" 那条记录的 AVAL 复制成 BASE。
#   要点：
#     - 第一个参数是数据集 advs
#     - by_vars    = exprs(STUDYID, USUBJID, PARAMCD, BASETYPE)
#     - source_var = AVAL，new_var = BASE
#     - filter     = ABLFL == "Y"（只用基线那条作为来源）
#   参考：紧接着下面的 BNRIND（基线正常范围指示）用的就是同一个函数，可照抄结构。
# 👉 把下面的占位赋值改成 advs <- derive_var_base(advs, ...)
advs <- advs   # 占位：填好上面 TODO 后改成 advs <- derive_var_base(advs, ...)
...
# TODO 3: 派生相对基线的变化量 CHG（= AVAL - BASE）
#   CHG 依赖 TODO 2 的 BASE，所以要先填好 BASE。
#   用 restrict_derivation 包住 derive_var_chg，只对治疗后记录（AVISITN > 0）计算：
#     - 第一个参数是数据集 advs
#     - derivation = derive_var_chg
#     - filter     = AVISITN > 0
#   参考：紧接着下面的 PCHG（百分变化）结构完全一样，只是 derivation 换成 derive_var_pchg。
# 👉 把下面的占位赋值改成 advs <- restrict_derivation(advs, derivation = derive_var_chg, filter = AVISITN > 0)
advs <- advs   # 占位：填好上面 TODO 后改成 advs <- restrict_derivation(...)
