...
adsl_raw <- adsl_ct %>%
  ...
  # TODO 1: 派生治疗开始时间 TRTSDTM/TRTSTMF（Treatment Start Datetime）
  #   用 derive_vars_merged，从 ex_ext 里取该受试者最早一次实际用药的开始时间。
  #   要点：
  #     - dataset_add = ex_ext
  #     - filter_add  = 只保留实际用药记录（EXDOSE > 0，或安慰剂 EXDOSE==0 且 EXTRT 含 "PLACEBO"）且 EXSTDTM 非缺失
  #     - new_vars    = exprs(TRTSDTM = EXSTDTM, TRTSTMF = EXSTTMF)
  #     - order       = exprs(EXSTDTM, EXSEQ)，mode = "first"（取最早）
  #     - by_vars     = exprs(STUDYID, USUBJID)
  # 👉 在这里补一段 derive_vars_merged(...)
  ...
  # TODO 2: 派生安全性人群标志 SAFFL（Safety Population Flag）
  #   安全性人群 = 至少有一次实际用药记录。用 derive_var_merged_exist_flag。
  #   要点：
  #     - dataset_add = ex
  #     - by_vars     = exprs(STUDYID, USUBJID)
  #     - new_var     = SAFFL，false_value = "N"，missing_value = "N"
  #     - condition   = 有实际用药（EXDOSE > 0，或安慰剂 EXDOSE==0 且 EXTRT 含 "PLACEBO"）
  # 👉 在这里补一段 derive_var_merged_exist_flag(...)
  identity()   # 占位：填好上面 TODO 后删掉这行 identity()
