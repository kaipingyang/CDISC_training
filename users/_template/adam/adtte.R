...
# TODO 1: 定义"死亡事件"来源 death_event
#   用 event_source 描述"哪些记录算作死亡事件、事件日期取哪一列"。
#   要点：
#     - dataset_name = "adrs"
#     - filter       = PARAMCD == "DEATH" & AVALC == "Y" & ANL01FL == "Y"
#     - date         = ADT
#     - set_values_to = exprs(EVNTDESC = "Death", SRCDOM = "ADRS", SRCVAR = "ADT")
#   参考：紧接着下面的 pd_event（疾病进展事件）用的就是同一个 event_source()，可照抄结构。
# 👉 把下面的 NULL 换成 event_source(...)
death_event <- NULL   # 占位：填好上面 TODO 后改成 death_event <- event_source(...)
...
# TODO 2: 派生总生存期 OS 参数（依赖 TODO 1 的 death_event，先填好它）
#   用 derive_param_tte 把"随机化 → 死亡/删失"的时间到事件行加进数据集。
#   要点：
#     - dataset_adsl      = adsl
#     - start_date        = RANDDT
#     - event_conditions  = list(death_event)
#     - censor_conditions = list(lastalive_censor, rand_censor)  # 顺序重要：先末次存活，再随机化日
#     - source_datasets   = list(adsl = adsl, adrs = adrs)
#     - set_values_to     = exprs(PARAMCD = "OS", PARAM = "Overall Survival")
#   参考：紧接着下面的 PFS 用的就是同一个 derive_param_tte()，可照抄结构。
# 👉 把下面的占位赋值改成 adtte <- derive_param_tte(...)
adtte <- adsl   # 占位：填好上面 TODO 后改成 adtte <- derive_param_tte(...)
